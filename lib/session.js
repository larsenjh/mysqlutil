...
module.exports = function (connectionParams) {
	...
	function preFillPool(amtToPrefill, cb) {
		async.each(
			new Array(amtToPrefill + 1).join(0).split(''),
			function (i, eCb) {
	...
	function addPingToConnection(conn) {
		conn.pingIntervalId = setInterval(conn.ping, connectionParams.connectionPingIntervalSeconds * 1000);
